...
def slice(  # pylint: disable=redefined-builtin,too-many-branches,too-many-statements
    ...
    *,
    ...
    if is_same_obs and is_same_vars:
        bdata = copy_adata(adata, name=name, share_derived=share_derived, top_level=top_level)
...
@utm.timed_call()
def copy_adata(
    adata: AnnData, *, name: Optional[str] = None, share_derived: bool = True, top_level: bool = True
) -> AnnData:
    """
...
def _get_o_data(
    ...
    *,
    ...
    if sum:
        ...
        data = _get_shaped_data(adata, "o", adata.obs, shape=(adata.n_obs,), name=name)
...
def _get_v_data(
    ...
    *,
    ...
    if sum:
        ...
        data = _get_shaped_data(adata, "v", adata.var, shape=(adata.n_vars,), name=name)
...
def _get_oo_data(
    ...
    *,
    ...
    data = _get_layout_data(adata, "oo", adata.obsp, shape=(adata.n_obs, adata.n_obs), name=name, layout=layout)
...
def _get_vv_data(
    ...
    *,
    ...
    data = _get_layout_data(adata, "vv", adata.varp, shape=(adata.n_vars, adata.n_vars), name=name, layout=layout)
...
def _get_oa_data(
    ...
    *,
    ...
    data = _get_layout_data(adata, "oa", adata.obsm, shape=(adata.n_obs, 0), name=name, layout=layout)
...
def _get_va_data(
    ...
    *,
    ...
    data = _get_layout_data(adata, "va", adata.varm, shape=(adata.n_vars, 0), name=name, layout=layout)
...
def _get_vo_data(
    ...
    *,
    ...
    data = _get_layout_data(adata, "vo", adata.layers, shape=(adata.n_obs, adata.n_vars), name=name, layout=layout)
...
def _get_vo_sum_data(
    ...
    if not hasattr(adata, "__derived__"):
        derived: Dict[str, utt.NumpyVector] = {}
...
def set_v_data(
    adata: AnnData, name: str, data: utt.NumpyVector, *, formatter: Optional[Callable[[Any], Any]] = None
) -> Any:
    """
...
def set_oo_data(
    adata: AnnData, name: str, data: utt.ProperMatrix, *, formatter: Optional[Callable[[Any], Any]] = None
) -> Any:
    """
